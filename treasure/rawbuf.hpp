...
struct raw_buf : private boost::noncopyable
{
  ...
  explicit raw_buf(char const * buf, int len = 0) : _data(NULL),_len(0)
  {
    if(0 == len) 
      len = strlen(buf)+1;
    do_malloc(len);
  ...
  void do_malloc(int len) {
    ...
    if(_data) {
      _len = len;
    } else g_warning("g_malloc0(%d) failed!", len);
  }
  raw_buf(raw_buf && rhs) {
  ...
  void expand(int len){
    ...
    if(data){
      ...
      g_free(_data); _data = data; _len = len;
    } else g_warning("g_malloc0(%d) failed!", len);
  }
  ...
  raw_buf & assign(raw_buf const & rhs){ 
    if(rhs._len <= _len){
      ...
      _len = rhs._len;
    } else g_warning("raw_buf::assign: can't not assign a bigger raw_buf to a small one!");
    return *this; 
  }
